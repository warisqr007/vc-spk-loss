...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        #required=True,
        default="/mnt/data1/waris/model_preprocessing/transformer-vc-vctk/bnfs",
    )
    parser.add_argument(
        "--wav_dir",
        type=str,
...
if __name__ == "__main__":
    ...
    # compute_bnf(**kwargs)
    # 2. Glob wav files
    wav_file_list = glob2.glob(f"{args.wav_dir}/**/*mic2.wav")
    print(f"Globbing {len(wav_file_list)} wav files.")
    Parallel(n_jobs=6)(delayed(compute_bnf)(args.output_dir,wav_file,args.train_config,args.model_file) for wav_file in tqdm(wav_file_list))
